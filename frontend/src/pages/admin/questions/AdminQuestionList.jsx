...
export default function AdminQuestionList({
  ...
  }, [debouncedSearch, subject.id, category, year, difficultyF, typeF, page, toast]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="soft-card p-4 flex flex-wrap gap-3 items-center">
        ...
        {(search || difficultyF || typeF) && (
          <Button variant="ghost" size="sm" onClick={() => { setSearch(''); setDifficultyF(''); setTypeF(''); setPage(1); }}>
            Clear
          </Button>
        )}
      </div>
                ...
                return (
                  <div key={q.id} className="flex items-start gap-3 px-5 py-4 hover:bg-surface transition-colors">
                    ...
                    <div className="flex gap-1.5 flex-shrink-0 ml-auto">
                      <Link
                        to={`/admin/questions/${q.id}/edit?return=${encodeURIComponent(listPath)}`}
